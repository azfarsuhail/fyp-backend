...
def send_otp_email(email_to: str, otp_code: str) -> bool:
    """
    Send OTP code email to user for password reset.
    
    Args:
        email_to: Recipient's email address
        otp_code: Plain-text 6-digit OTP code
        
    Returns:
        True if email sent successfully, False otherwise
    """
    if not RESEND_API_KEY:
        print(f"[Email Service] Skipping OTP email to {email_to} - RESEND_API_KEY not configured")
        return False
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                <p>Hello,</p>
                <p>We received a request to reset your password. If you made this request, please use the following one-time password code:</p>
                ...
                <div class="warning">
                    <strong>⚠️ Important:</strong> This code will expire in <strong>5 minutes</strong> for security reasons. After that, you'll need to request a new code.
                </div>
                ...
                <ul style="color: #555;">
                    ...
                    <li>After 3 failed attempts, the code will be locked</li>
                </ul>
    ...
    try:
        ...
        print(f"[Email Service] OTP email sent to {email_to} with code: {otp_code}")
        return True
        
    except Exception as e:
        print(f"[Email Service] Failed to send OTP email to {email_to}: {str(e)}")
        return False
